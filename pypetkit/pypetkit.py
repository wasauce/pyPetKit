...
class PetKitAPI:
    ...
    def get_all_devices(self):
        ...
        result = requests.get(
            self._apiServerBaseURL + API_FEEDERMINI_DEVICE_PATH, headers=custom_headers
        )

        try:
            for item in result.json().get("result", []):
                feeder = PetKitDevice(self._access_token, item, self._apiServerBaseURL)
                self.feeders[feeder.id] = feeder

        except (KeyError, TypeError) as err:
            _LOGGER.error("Error requesting device from PetKit: {}".format(err))
    ...
    def request_token(self):
        ...
        result = requests.post(
            self._apiServerBaseURL + API_LOGIN_PATH, data=params, headers=custom_headers
        )

        try:
            createdAt = datetime.strptime(
                result.json()["result"]["session"]["createdAt"], "%Y-%m-%dT%H:%M:%S.%fZ"
            )
    ...
    def get_api_server_by_country(self, country="US"):
        ...
        try:
            ...
            country_data = next(
                (
                    item
                    for item in result_json["result"]["list"]
                    if item["id"] == country
                ),
                None,
            )
